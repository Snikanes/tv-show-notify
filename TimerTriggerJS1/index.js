...
module.exports = async function(context) {
    ...
    try {
        ...
        const episodes = await Promise.all(shows.map(async show => (await tvApi.getEpisodes(show.data.id)).data.map(json => Episode.fromJson(json, show.data.id, show.data.seriesName, show.data.airsTime))))
